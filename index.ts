...
const nostrRelays = ['wss://relay.damus.io', 'wss://nos.lol', 'wss://relay.snort.social', 'wss://purplerelay.com'];
let nostrSub: any = null;
let nostrUserSk: Uint8Array | null = null;
let nostrUserPk: string | null = null;
...
async function openApp(appName: string): Promise<void> {
    ...
    if (iconElement) {
        ...
        if(span) title = span.textContent || appName;
         ...
         switch(appName) {
            ...
            case 'nostalgia': iconSrc = 'https://win98icons.alexmeub.com/icons/png/outlook_express-4.png'; title = 'Nostalgia'; break;
         }
    }
    ...
    else if (appName === 'nostalgia') {
        initNostalgiaApp(windowElement);
    }
}
...
// --- Nostalgia Nostr App Functions ---

function cleanupNostalgiaApp() {
    if (nostrSub) {
        nostrSub.close();
        nostrSub = null;
    }
...
async function initNostalgiaApp(windowElement: HTMLDivElement) {
    const feedList = windowElement.querySelector('#nostalgia-feed-list') as HTMLDivElement;
    const statusEl = windowElement.querySelector('.nostalgia-status') as HTMLDivElement;
    const loginBtn = windowElement.querySelector('#nostalgia-btn-login') as HTMLButtonElement;
    const skInput = windowElement.querySelector('#nostalgia-sk') as HTMLInputElement;
    const postBtn = windowElement.querySelector('#nostalgia-btn-post') as HTMLButtonElement;
    const postText = windowElement.querySelector('#nostalgia-post-text') as HTMLTextAreaElement;
    const feedBtn = windowElement.querySelector('#nostalgia-btn-feed') as HTMLButtonElement;
    const profileBtn = windowElement.querySelector('#nostalgia-btn-profile') as HTMLButtonElement;
    const relaysBtn = windowElement.querySelector('#nostalgia-btn-relays') as HTMLButtonElement;

    if (!feedList || !statusEl || !loginBtn || !skInput || !postBtn || !postText) return;
    ...
    const notes: any[] = [];
    const updateFeed = (events: any[]) => {
        feedList.innerHTML = '';
        if (events.length === 0) {
            feedList.innerHTML = '<div class="nostalgia-loading">No notes found.</div>';
            return;
        }
        ...
        events.forEach(event => {
            ...
            noteDiv.className = 'nostalgia-note';
            
            const author = event.pubkey.substring(0, 8) + "...";
            const date = new Date(event.created_at * 1000).toLocaleString();
            
            noteDiv.innerHTML = `
                <div class="nostalgia-note-header">
                    <span class="nostalgia-note-author">@${author}</span>
                    <span class="nostalgia-note-date">${date}</span>
                </div>
                <div class="nostalgia-note-content">${escapeHtml(event.content)}</div>
                <div class="nostalgia-note-footer">
                    <span class="nostalgia-btn-zap" data-id="${event.id}">⚡ Zap</span>
                    <span>💬 Reply</span>
                    <span>🔄 Repost</span>
                </div>
            `;
            
            const zapBtn = noteDiv.querySelector('.nostalgia-btn-zap');
            zapBtn?.addEventListener('click', () => {
                alert(`Zapping note ${event.id.substring(0, 8)}... (Integration with Lightning Wallet required)`);
            });

            feedList.appendChild(noteDiv);
        });
    };

    const appendNote = (event: any) => {
    ...
    const loadFeed = () => {
        if (nostrSub) nostrSub.close();
        feedList.innerHTML = '<div class="nostalgia-loading">Fetching global feed...</div>';
        
        nostrSub = nostrPool!.subscribeMany(
            nostrRelays,
            { kinds: [1], limit: 50 },
            {
                onevent(event) {
                ...
                oneose() {
                    statusEl.textContent = "Connected to relays";
                }
            }
        );
    };
    ...
    loginBtn.addEventListener('click', () => {
        ...
        if (!sk) {
            alert("Please enter an nsec or hex private key.");
            return;
        }
        ...
        try {
            ...
            if (sk.startsWith('nsec')) {
                ...
                nostrUserSk = data as Uint8Array;
            } else {
                nostrUserSk = hexToBytes(sk);
            }
            
            nostrUserPk = getPublicKey(nostrUserSk);
            const npub = nip19.npubEncode(nostrUserPk);
            alert(`Logged in as ${npub.substring(0, 12)}...`);
            skInput.value = "********";
    ...
    postBtn.addEventListener('click', async () => {
        if (!nostrUserSk) {
            alert("Please login first with your nsec.");
            return;
        }
        ...
        try {
            ...
            const event = finalizeEvent(eventTemplate, nostrUserSk);
    ...
    profileBtn.addEventListener('click', () => {
        ...
        const npub = nip19.npubEncode(nostrUserPk);
        alert(`Your npub: ${npub}\n\n(Profile editing coming soon in Nostalgia v2.0)`);
    });
